"""Command-line interface for FastBotty"""
...
def init(project_name: str) -> None:
    """Initialize a new FastBotty project"""
    ...
    config_content = """# FastBotty Configuration
...
"""
    ...
    main_content = '''"""FastBotty server entry point"""
...
'''
    ...
    # Get current fastbotty version from development
    import subprocess
    import sys
    ...
    try:
        ...
        result = subprocess.run(
            [sys.executable, "-c", "import fastbotty; print(fastbotty.__version__)"],
            capture_output=True,
            text=True,
    ...
    (project_path / "requirements.txt").write_text(f"fastbotty>={current_version}\n")

    readme = f"""# {project_name}

FastBotty notification service
...
def run(config: str, host: str, port: int, reload: bool) -> None:
    """Run the FastBotty server"""
    if not Path(config).exists():
        click.echo(f"Error: Config file '{config}' not found", err=True)
        click.echo("Run 'fastbotty init <project_name>' to create a new project")
        return
    ...
    click.echo(f"Starting FastBotty server on {final_host}:{final_port}")

    uvicorn.run(
        "fastbotty.server.app:create_app",
        host=final_host,
        port=final_port,
        reload=reload,
        factory=True,
